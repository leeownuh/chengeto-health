...
async function main() {
  ...
  const users = [
    { _id: ids.admin, email: 'admin@chengeto.health', password: DEMO_PASSWORD, firstName: 'System', lastName: 'Administrator', phone: '+263771000001', role: 'admin', permissions: rolePermissions.admin, status: 'active', emailVerified: true, phoneVerified: true, mfaEnabled: false, loginAttempts: 0, assignedPatients: [], lastLogin: { timestamp: hoursAgo(4), ipAddress: '127.0.0.1', userAgent: 'Demo seed' }, createdAt: daysAgo(30), updatedAt: now },
    { _id: ids.chw, email: 'chw1@chengeto.health', password: DEMO_PASSWORD, firstName: 'Nyasha', lastName: 'Mukamuri', phone: '+263771000011', role: 'chw', permissions: rolePermissions.chw, status: 'active', emailVerified: true, phoneVerified: true, mfaEnabled: false, loginAttempts: 0, assignedPatients: ids.patients, ward: 'Ward 16', district: 'Harare', lastLogin: { timestamp: hoursAgo(6), ipAddress: '127.0.0.1', userAgent: 'Demo seed' }, createdAt: daysAgo(28), updatedAt: now },
    { _id: ids.caregiver, email: 'caregiver1@example.com', password: DEMO_PASSWORD, firstName: 'Tariro', lastName: 'Moyo', phone: '+263771000021', role: 'caregiver', permissions: rolePermissions.caregiver, status: 'active', emailVerified: true, phoneVerified: true, mfaEnabled: false, loginAttempts: 0, assignedPatients: ids.patients, isPrimaryCaregiver: true, specializations: ['Medication adherence', 'Home visits'], lastLogin: { timestamp: hoursAgo(3), ipAddress: '127.0.0.1', userAgent: 'Demo seed' }, createdAt: daysAgo(21), updatedAt: now },
    { _id: ids.clinician, email: 'clinician1@chengeto.health', password: DEMO_PASSWORD, firstName: 'Dr. Farai', lastName: 'Mlambo', phone: '+263771000031', role: 'clinician', permissions: rolePermissions.clinician, status: 'active', emailVerified: true, phoneVerified: true, mfaEnabled: false, loginAttempts: 0, specializations: ['Geriatrics'], lastLogin: { timestamp: hoursAgo(8), ipAddress: '127.0.0.1', userAgent: 'Demo seed' }, createdAt: daysAgo(35), updatedAt: now },
    { _id: ids.family, email: 'family1@example.com', password: DEMO_PASSWORD, firstName: 'Kudzai', lastName: 'Moyo', phone: '+263771000041', role: 'family', permissions: rolePermissions.family, status: 'active', emailVerified: true, phoneVerified: true, mfaEnabled: false, loginAttempts: 0, linkedPatients: ids.patients.map((patient) => ({ patient, relationship: 'child', accessLevel: 'full' })), lastLogin: { timestamp: daysAgo(2), ipAddress: '127.0.0.1', userAgent: 'Demo seed' }, createdAt: daysAgo(18), updatedAt: now },
    { _id: ids.auditor, email: 'auditor@chengeto.health', password: DEMO_PASSWORD, firstName: 'Audit', lastName: 'Officer', phone: '+263771000051', role: 'auditor', permissions: rolePermissions.auditor, status: 'active', emailVerified: true, phoneVerified: true, mfaEnabled: false, loginAttempts: 0, lastLogin: { timestamp: daysAgo(5), ipAddress: '127.0.0.1', userAgent: 'Demo seed' }, createdAt: daysAgo(40), updatedAt: now }
  ];
  ...
  const auditLogs = [
    buildAuditLog(makeStampedId('AUD', 1), AUDIT_ACTIONS.LOGIN, 'authentication', actor(ids.admin, 'admin@chengeto.health', 'admin'), { type: 'user', id: ids.admin, model: 'User', description: 'Admin login' }, { message: 'Administrator reviewed deployment health.' }, hoursAgo(12)),
    buildAuditLog(makeStampedId('AUD', 2), AUDIT_ACTIONS.PATIENT_CREATE, 'patient_management', actor(ids.admin, 'admin@chengeto.health', 'admin'), { type: 'patient', id: ids.patients[0], model: 'Patient', description: 'Patient enrolled' }, { message: 'Seeded patient profile.' }, daysAgo(18)),
    buildAuditLog(makeStampedId('AUD', 3), AUDIT_ACTIONS.DEVICE_REGISTER, 'device', actor(ids.admin, 'admin@chengeto.health', 'admin'), { type: 'device', id: ids.devices[0], model: 'IoTDevice', description: devices[0].deviceId }, { message: 'Wearable provisioned.' }, daysAgo(17)),
    buildAuditLog(makeStampedId('AUD', 4), AUDIT_ACTIONS.CHECKIN_CREATE, 'checkin', actor(ids.caregiver, 'caregiver1@example.com', 'caregiver'), { type: 'checkin', id: ids.checkins[0], model: 'CheckIn', description: 'Morning check-in' }, { message: 'Routine visit completed.' }, hoursAgo(5.5)),
    buildAuditLog(makeStampedId('AUD', 5), AUDIT_ACTIONS.ALERT_TRIGGER, 'alert', actor(ids.admin, 'admin@chengeto.health', 'admin'), { type: 'alert', id: ids.alerts[0], model: 'Alert', description: 'Critical vitals alert' }, { message: 'Telemetry anomaly created alert.' }, minutesAgo(36)),
    buildAuditLog(makeStampedId('AUD', 6), AUDIT_ACTIONS.ALERT_ESCALATE, 'alert', actor(ids.caregiver, 'caregiver1@example.com', 'caregiver'), { type: 'alert', id: ids.alerts[0], model: 'Alert', description: 'Escalated to clinician' }, { message: 'Escalated after persistent abnormal readings.' }, minutesAgo(20)),
    buildAuditLog(makeStampedId('AUD', 7), AUDIT_ACTIONS.DATA_ACCESS, 'data_access', actor(ids.clinician, 'clinician1@chengeto.health', 'clinician'), { type: 'patient', id: ids.patients[2], model: 'Patient', description: 'Critical patient review' }, { message: 'Clinician opened patient record.' }, minutesAgo(15)),
    buildAuditLog(makeStampedId('AUD', 8), AUDIT_ACTIONS.LOGIN, 'authentication', actor(ids.family, 'family1@example.com', 'family'), { type: 'user', id: ids.family, model: 'User', description: 'Family portal login' }, { message: 'Family member accessed dashboard.' }, daysAgo(2))
  ];
